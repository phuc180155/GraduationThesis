...
class ImageEmbedder(nn.Module):
    ...
    def forward(self, img):
        x = self.wadd.extract_features_at_block(img, self.selected_block)
        '''
        x_scaled = []
...
class CrossWADDViT(nn.Module):
    def __init__(
            self,
            *,
            image_size=224,num_classes=1,sm_dim=192,selected_sm_block=5,lg_dim=384,
            selected_lg_block=2,sm_patch_size=7,sm_enc_depth=2,sm_enc_heads=8,
        ...
        self.sm_dim = sm_dim
        self.sm_blocks = selected_sm_block
        self.lg_dim = lg_dim
        self.lg_blocks = selected_lg_block
...
if __name__ == "__main__":
    model = CrossWADDViT(image_size=256,sm_patch_size=8,lg_patch_size=16)
    import torchsummary
    torchsummary.summary(model,(3,256,256))


 # python train.py --train_set ../df_in_the_wild/image_jpg/train/ --val_set ../df_in_the_wild/image_jpg/test/ --batch_size 24 --niter 50 --image_size 256 --workers 24 --checkpoint cross_waddvit_1_df_inthewild_checkpoint --gpu_id 0 --lr 1e-4 --print_every 20000 crosswaddvit --sm_patch_size 8 --lg_patch_size 16 --selected_sm_block 5 --selected_lg_block 1