...
def train_cnn(model,criterion,train_set = '../../extract_raw_img',val_set ='../../extract_raw_img',image_size=256,\
    ...
    # import time
    text_writer = open(os.path.join(checkpoint, 'train.csv'), 'w')
    text_writer_epoch = open(os.path.join(checkpoint, 'train_epoch.csv'), 'w')
    ...
    for epoch in range(epochs):
        ...
        text_writer_epoch.write('Epoch %.4f, running loss  %.4f \n' % (
            epoch, running_loss / len(dataloader_train)))
        running_loss = 0
        accuracy_score__= eval_train(model ,dataloader_val,device,criterion,text_writer_epoch,adj_brightness=adj_brightness, adj_contrast=adj_brightness)
