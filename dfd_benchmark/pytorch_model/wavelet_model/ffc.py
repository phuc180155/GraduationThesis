...
# https://github.com/saic-mdal/lama/blob/cf9b1dd93bdda97b3b87354a6063003a6a5e77d0/saicinpainting/training/modules/ffc.py#L357
import numpy as np
import torch
...
class FourierUnit(nn.Module):
    ...
    def forward(self, x):
        ...
        r_size = x.size()
        # (batch, c, h, w/2+1, 2)
        # print("r_size : ",r_size)   #         [2, 3, 128, 128]
        fft_dim = (-3, -2, -1) if self.ffc3d else (-2, -1)
        ffted = torch.fft.rfftn(x, dim=fft_dim, norm=self.fft_norm)
        # print("101  :",ffted.size())    #        [2, 3, 128, 65]

        ffted = torch.stack((ffted.real, ffted.imag), dim=-1)
        # print("104  :",ffted.size())    #  [2, 3, 128, 65, 2]

        ffted = ffted.permute(0, 1, 4, 2, 3).contiguous()  # (batch, c, 2, h, w/2+1)
        # print("106  :",ffted.size())  #  [2, 3, 2, 128, 65]

        ffted = ffted.view((batch, -1,) + ffted.size()[3:])
        # print("107  :",ffted.size())    # [2, 6, 128, 65]
...
class FFC(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
                 ratio_gin, ratio_gout, stride=1, padding=2,
                 dilation=1, groups=1, bias=False, enable_lfu=True,
                 padding_type='same', gated=False, **spectral_kwargs):
        ...
        in_cg = in_channels
        in_cl = in_channels
        out_cg = in_channels //2
        out_cl = in_channels //2
    ...
    def forward(self, x):
        x_l, x_g = x if type(x) is tuple else (x, x)
        out_xl, out_xg = 0, 0
        ...
        # print("out_xl   :",out_xl.size())
        # out_xg = self.convl2g(x_l) + self.convg2g(x_g)
        out_xg = self.convg2g(x_g)
        # print("out_xg   :",out_xg.size())
...
class FFC_BN_ACT(nn.Module):
    ...
    def __init__(self, in_channels, out_channels,
                 kernel_size=3, ratio_gin=1.0, ratio_gout=1.0,
        ...
        global_channels = int(out_channels // 2)
