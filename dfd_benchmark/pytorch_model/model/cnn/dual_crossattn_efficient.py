...
class DualCrossAttnEfficient(nn.Module):
    def __init__(self, channels=1280,\
                 ...
                 mlp_dim=2048,\
                 dropout=0.15,version="cross_attention-spatial-cat",weight=0.5,freeze=0):  
    ...
    def get_feature_extractor(self, architecture="efficient_net", freeze=0, pretrained="", num_classes=1, in_channels=3):
        ...
        if architecture == "efficient_net":
            if pretrained == "":
                extractor = EfficientNet.from_pretrained('efficientnet-b0', num_classes=num_classes,in_channels = in_channels,pretrained=False)
            else:
                extractor = EfficientNet.from_pretrained('efficientnet-b7', num_classes=num_classes,in_channels = in_channels,pretrained=False)
                # Load checkpoint
