...
class Statistics(object):
    ...
    def _compute_statistics(self):
        for encoded in self.datasets:
            ...
            images = map(lambda d: load_image(d, grayscale=not self.color), dataset)
            images_dct = map(dct2, images)
            print("images_dct: ", images_dct)
...
def main():
    ...
    # print(args)
    DATASETS = ["//mnt/disk1/doan/phucnp/Dataset/ff/image/component/df/test/0_real,REAL", \
        "/mnt/disk1/doan/phucnp/Dataset/ff/image/component/df/test/1_df,FAKE"]
    amount = 5000
    color = False
    output = 'output'

    Statistics(amount, DATASETS, color, output).compute_and_plot()
...
def parse_args():
    ...
    parser.add_argument(
        "--color", "-c", help="Plot for each color channel seperate.", default=True, action="store_true")
