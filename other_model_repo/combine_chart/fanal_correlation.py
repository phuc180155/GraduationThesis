...
def heatmap(data, row_labels, col_labels, ax=None,
    ...
    **kwargs
    ...
    # import pdb 
    # pdb.set_trace()
    im = ax.imshow(new_data, vmin=50., vmax=100, **kwargs)
    ...
    ax.set_yticklabels(row_labels, fontweight='bold', fontsize=18)
    # ax.set_yticklabels(row_labels, fontweight='bold', fontsize=20)
    # ax.set_xlabel('vinhsuhi')
    # ax.set_title(title)
    # ax.axis(ymin=0, ymax=0, xmin=0, xmax=0)

    # Let the horizontal axes labeling appear on top.
    # ax.tick_params(axis='x', which='major', labelsize=8)

    # Rotate the tick labels and set their alignment.
    plt.setp(ax.get_xticklabels(),ha="center", rotation=90, fontsize=20)
    # plt.setp(ax.get_xticklabels(),ha="center", rotation=90, fontsize=22)
    ...
    **kwargs
    ...
    for i in range(data.shape[0]):
        for j in range(data.shape[1]):
            ...
            lol = valfmt(data[i, j], None)
            if data[i,j] == 0.321:
                lol = "0.0"
            # text = im.axes.text(j, i * grid_size_2,lol , weight='bold', fontsize=11, **kw)
            text = im.axes.text(j, i * grid_size_2,lol , weight='bold', fontsize=16, **kw)
            texts.append(text)
...
with open(data_path, 'r', encoding='utf-8') as file:
    for line in file:
        ...
        for ele in data_line:
            if not 'x' in ele:
                new_data_line.append(float("{:.1f}".format(float(ele))))
            else:
                new_data_line.append(0)
...
gen_teches = ["Meso4", "Capsule", "XceptionNet", "GAN-fp", "FDBD", "HPBD", "VA"]
# detech_teches = 
detech_techs = ['Deepfake', '3DMM', 'FaceSwap-2D', 'FaceSwap-3D', 'MonkeyNet', 'ReenactGAN', 'StarGAN','X2Face']
noises = ['Deepfake', '3DMM', 'FaceSwap-2D', 'FaceSwap-3D', 'MonkeyNet', 'ReenactGAN', 'StarGAN','X2Face']
...
im, cbar = heatmap(data_block, detech_techs, noises,  ax=ax,
                cmap="GnBu", cbarlabel="harvest [t/year]",
