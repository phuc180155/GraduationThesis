...
class PairwiseSingleFFTMagnitudeImageDataset(Dataset):
    def __init__(self, path=None, image_size=128, transform=None, transform_fft=None, should_invert=True,shuffle=True, adj_brightness=None, adj_contrast=None, dset=None, freq_stream='magnitude'):
        self.path = path
    ...
    def __getitem__(self, index):
        ...
        img0, img1 = self.__data_generation(img0_path, img1_path)
        ...
        if img0_label != img1_label:
            ...
            label_contrastive = 0
        return img0, img0_label, img1, img1_label, label_contrastive

    def __data_generation(self, img0_path, img1_path):
        img0 = self.__get_image(img0_path)
        img1 = self.__get_image(img1_path)
        return img0, img1
    ...
    def __get_image(self, img_path):
       # Read image in RGB and resize to (image_size, image_size)
        ...
        ############ Make FFT image ############
        if self.freq_stream in ['magnitude', 'phase']:
            ...
            if self.freq_stream == 'magnitude':
                spectrum = np.log(np.abs(fshift))
            else:
                spectrum = np.angle(fshift)

            spectrum = cv2.resize(spectrum, (self.image_size,self.image_size))
            spectrum = np.array([spectrum])
            spectrum = np.transpose(spectrum, (1, 2, 0))    # From C, H, W =>  H, W, C
            
            if self.transform_fft is not None:
                spectrum = self.transform_fft(spectrum)
        return PIL_img if self.freq_stream == 'rgb' else spectrum
