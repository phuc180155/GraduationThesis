...
class PairwiseDualFFTMagnitudeImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if img0_label != img1_label:
            ...
            label_contrastive = 0
        return PIL_img0, fft_img0, img0_label, PIL_img1, fft_img1, img1_label, label_contrastive
...
class PairwiseDualFFTMagnitudeFeatureDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        PIL_img0, PIL_img1, fft_fea0, fft_fea1 = self.__data_generation(img0_path, img1_path)
        # print(img0_path, img1_path)
        # print(img0_label, img1_label)
        ...
        if img0_label != img1_label:
            ...
            label_contrastive = 0
        return PIL_img0, fft_fea0, img0_label, PIL_img1, fft_fea1, img1_label, label_contrastive

    def __data_generation(self, img0_path, img1_path):
        PIL_img0, fft_fea0 = self.__get_dual_image_feature(img0_path)
        PIL_img1, fft_fea1 = self.__get_dual_image_feature(img1_path)
        return PIL_img0, PIL_img1, fft_fea0, fft_fea1


    def __get_dual_image_feature(self, img_path):
       # Read image in RGB and resize to (image_size, image_size)
        img = cv2.imread(img_path)
        img = cv2.cvtColor(img,cv2.COLOR_BGR2RGB)
        img = cv2.resize(img,(self.image_size,self.image_size))
        ...
        if self.transform_fft is not None:
            magnitude_spectrum = self.transform_fft(psd1D)
        return PIL_img, psd1D
