...
class TriplewiseDualFFTMagnitudeImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        while True:
            #keep looping till the same class image is found
            img2_index = random.choice(self.indexes)
            if index == img2_index:
                continue
            img2_path = self.data_path[img2_index]
            img2_label = self.find_label(img2_path)
            if img0_label != img2_label:
                break

        PIL_img0, PIL_img1, PIL_img2, fft_img0, fft_img1, fft_img2 = self.__data_generation(img0_path, img1_path, img2_path)
        return PIL_img0, fft_img0, img0_label, PIL_img1, fft_img1, img1_label, PIL_img2, fft_img2, img2_label

    def __data_generation(self, img0_path, img1_path, img2_path):
        PIL_img0, fft_img0 = self.__get_dual_images(img0_path)
        PIL_img1, fft_img1 = self.__get_dual_images(img1_path)
        PIL_img2, fft_img2 = self.__get_dual_images(img2_path)
        return PIL_img0, PIL_img1, PIL_img2, fft_img0, fft_img1, fft_img2
