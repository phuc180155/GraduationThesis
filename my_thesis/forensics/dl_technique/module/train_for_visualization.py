            ...
            for inputs0, fft_imgs0, labels0, inputs1, fft_imgs1, labels1, labels_contrastive in tqdm(dataloader_train):
                ...
                if eval_per_iters != -1:
                    if global_step % eval_per_iters == 0:
                        model.eval()
                        # Eval validation set
                        val_contrastive_loss, val_bce_loss, val_total_loss, val_mac_acc, val_mic_acc, val_reals, val_fakes, val_micros, val_macros = eval_kfold_pairwise_image_stream_twooutput(model, weight_importance, dataloader_val, device, ce_loss, contrastive_loss, adj_brightness=adj_brightness, adj_contrast=adj_brightness)
                        ...
                        # Eval test set
                        test_contrastive_loss, test_bce_loss, test_total_loss, test_mac_acc, test_mic_acc, test_reals, test_fakes, test_micros, test_macros = eval_kfold_pairwise_image_stream_twooutput(model, weight_importance, dataloader_test, device, ce_loss, contrastive_loss, adj_brightness=adj_brightness, adj_contrast=adj_brightness)
...
def eval_kfold_pairwise_image_stream(model, weight_importance, dataloader, device, bce_loss, contrastive_loss, adj_brightness=1.0, adj_contrast=1.0):
    """ Evaluate model with dataloader
    ...
    begin = time.time()
    with torch.no_grad():
        for inputs0, labels0, inputs1, labels1, labels_contrastive in dataloader:
            # Push to device
            y_label.extend(labels0.cpu().numpy().astype(np.float64))
            inputs0, labels0 = inputs0.float().to(device), labels0.float().to(device)
            inputs1, labels1 = inputs1.float().to(device), labels1.float().to(device)
            labels_contrastive = labels_contrastive.float().to(device)
            
            # Forward
            embedding_0, logps0, embedding_1, logps1  = model.forward(inputs0, inputs1)     # Shape (32, 1)
...
def train_kfold_pairwise_image_stream(model_, freq_stream='magnitude', what_fold='all', n_folds=5, use_trick=True, weight_importance=2, margin=2, train_dir = '', val_dir ='', test_dir= '', image_size=128, lr=3e-4, division_lr=True, use_pretrained=False,\
              batch_size=16, num_workers=8, checkpoint='', resume='', epochs=30, eval_per_iters=-1, seed=0, \
              adj_brightness=1.0, adj_contrast=1.0, es_metric='val_loss', es_patience=5, model_name="pairwise_cnn_cmultivit", args_txt="", augmentation=True):
    ...
    for fold_idx in range(n_folds):
        ...
        dataloader_train, dataloader_val, num_samples = generate_dataloader_single_cnn_stream_for_kfold_pairwise(train_dir, trainset, valset, image_size, batch_size, num_workers, freq_stream=freq_stream, augmentation=augmentation)
        dataloader_test = generate_test_dataloader_single_cnn_stream_for_kfold_pairwise(test_dir, image_size, batch_size, num_workers, freq_stream)    
        ...
        for epoch in range(init_epoch, epochs):
            ...
            model.train()
            print("Training...")
            for inputs0, labels0, inputs1, labels1, labels_contrastive in tqdm(dataloader_train):
                global_step += 1
                # Push to device
                # print("input shape: ", inputs0.shape, inputs1.shape)
                inputs0, labels0 = inputs0.float().to(device), labels0.float().to(device)
                inputs1, labels1 = inputs1.float().to(device), labels1.float().to(device)
                labels_contrastive = labels_contrastive.float().to(device)
                # Clear gradient after a step
                ...
                if eval_per_iters != -1:
                    if global_step % eval_per_iters == 0:
                        model.eval()
                        # Eval validation set
                        val_contrastive_loss, val_bce_loss, val_total_loss, val_mac_acc, val_mic_acc, val_reals, val_fakes, val_micros, val_macros = eval_kfold_pairwise_image_stream(model, weight_importance, dataloader_val, device, bce_loss, contrastive_loss, adj_brightness=adj_brightness, adj_contrast=adj_brightness)
                        save_result_for_pairwise(step_val_writer, log, global_step, global_contrastive_loss/global_step, global_bce_loss/global_step, global_total_loss/global_step, global_acc/global_step, val_contrastive_loss, val_bce_loss, val_total_loss, val_mac_acc, val_mic_acc, val_reals, val_fakes, val_micros, val_macros, is_epoch=False, phase="val")
                        # Eval test set
                        test_contrastive_loss, test_bce_loss, test_total_loss, test_mac_acc, test_mic_acc, test_reals, test_fakes, test_micros, test_macros = eval_kfold_pairwise_image_stream(model, weight_importance, dataloader_test, device, bce_loss, contrastive_loss, adj_brightness=adj_brightness, adj_contrast=adj_brightness)
                        save_result_for_pairwise(step_test_writer, log, global_step, global_contrastive_loss/global_step, global_bce_loss/global_step, global_total_loss/global_step, global_acc/global_step, test_contrastive_loss, test_bce_loss, test_total_loss, test_mac_acc, test_mic_acc, test_reals, test_fakes, test_micros, test_macros, is_epoch=False, phase="test")
                        # Save model:
                        step_model_saver(global_step, [val_bce_loss, val_total_loss, test_mic_acc], step_ckcpoint, model)
                        step_model_saver.save_last_model(step_ckcpoint, model, global_step)
