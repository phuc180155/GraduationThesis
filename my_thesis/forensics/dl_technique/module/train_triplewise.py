...
################# DUAL STREAM
#############################################
def define_log_writer_for_triplewise(checkpoint: str, resume: str, args_txt:str, model: Tuple[torch.nn.Module, str, int]):
    ...
    batch_contrast_writer = open(osp.join(ckc_pointdir, 'batch tripletloss loss.csv'), 'w')
    ...
    def make_sub_checkpoint(ckc_pointdir: str, eval_type="epoch", write_mode='w'):
        ...
        val_header = "{}, Train tripletloss loss, Train bce loss, Train total loss, Train accuracy,".format(eval_type) +\
                    " Val tripletloss loss, Val bce loss, Val total loss, Val accuracy," +\
        ...
        test_header = "{}, Train tripletloss loss, Train bce loss, Train total loss, Train accuracy,".format(eval_type) +\
                    " Test tripletloss loss, Test bce loss, Test total loss, Test accuracy," +\
...
def save_result_for_triplewise(text_writer, log, iteration, train_triplet_loss, train_bce_loss, train_total_loss, train_acc, val_triplet_loss, val_bce_loss, val_total_loss, val_mac_acc, val_mic_acc, reals, fakes, micros, macros, is_epoch=True, phase='val'):
    #
    ...
    result += "PHASE {} ##### Train c_loss: {:.4f}, b_loss: {:.4f}, t_loss: {:.4f} --- Train accuracy: {:.4f} --- ".format(phase, train_triplet_loss, train_bce_loss, train_total_loss, train_acc)
    result += "{} c_loss: {:.4f}, b_loss: {:.4f}, t_loss: {:.4f} --- {} macro acc: {:.4f} --- {} micro acc: {:.4f} --- ".format(phase, val_triplet_loss, val_bce_loss, val_total_loss, phase, val_mac_acc, phase, val_mic_acc)
    ...
    # Save text result 
    save_txt  = "{},{:.6f},{:.6f},{:.6f},{:.6f},".format(iteration, train_triplet_loss, train_bce_loss, train_total_loss, train_acc)
    save_txt += "{:.6f},{:.6f},{:.6f},{:.6f},".format(val_triplet_loss, val_bce_loss, val_total_loss, val_mic_acc)
    ...
    scalar_dict_triplet_loss = {
        "train tripletloss loss": train_triplet_loss, 
        "{} tripletloss loss".format(phase): val_triplet_loss
    }
    ...
    log.write_scalars(scalar_dict=scalar_dict_triplet_loss, global_step=iteration, tag="tripletloss Loss "+eval_type)
...
def eval_triplewise_dual_stream(model, weight_importance, dataloader, device, bce_loss, triplet_loss, adj_brightness=1.0, adj_contrast=1.0):
    ...
    with torch.no_grad():
        for inputs0, fft_imgs0, labels0, inputs1, fft_imgs1, labels1, inputs2, fft_imgs2, labels2 in dataloader:
            ...
            # Forward
            embedding_0, logps0, embedding_1, logps1, embedding_2, logps2 = model.forward(inputs0, fft_imgs0, inputs1, fft_imgs1, inputs2, fft_imgs2)     # Shape (32, 1)
            logps0 = logps0.squeeze()                     # Shape (32, )
            logps1 = logps1.squeeze()
            logps2 = logps2.squeeze()
            ...
            tripletloss = triplet_loss(embedding_0, embedding_1, embedding_2)
            batch_loss = weight_importance * bceloss_0 + tripletloss

            # Cumulate into running val loss
            triplet_loss_ += tripletloss.item()
    ...
    return triplet_loss_, bce_loss_, total_loss_, mac_accuracy, mic_accuracy, reals, fakes, micros, macros
    
def train_triplewise_dual_stream(model, weight_importance=2, margin=2, train_dir = '', val_dir ='', test_dir= '', image_size=128, lr=3e-4, division_lr=True, use_pretrained=False,\
              batch_size=16, num_workers=8, checkpoint='', resume='', epochs=30, eval_per_iters=-1, seed=0, \
              adj_brightness=1.0, adj_contrast=1.0, es_metric='val_loss', es_patience=5, model_name="triplewise_dual_cnn_feedforward", args_txt="", augmentation=True):
    
    # Generate dataloader train and validation 
    dataloader_train, dataloader_val = generate_dataloader_dual_cnn_stream_for_triplewise(train_dir, val_dir, image_size, batch_size, num_workers, augmentation=augmentation)
    dataloader_test = generate_test_dataloader_dual_cnn_stream_for_triplewise(test_dir, image_size, 2*batch_size, num_workers)
        ...
        try:
            ...
            if 'step' in checkpoint:
                ...
                with open(osp.join(checkpoint, 'global_acc_loss.txt'), 'r') as f:
                    ...
                    init_global_triplet_loss = float(line.split(',')[1])
                    init_global_total_loss = float(line.split(',')[3])

                print('Resume step: {} - in epoch: {} - lr: {} - global_acc: {} - global_bce_loss: {} - global_triplet_loss: {} - global_total_loss: {}'.format(init_step, init_epoch, init_lr, init_global_acc, init_global_bce_loss, init_global_triplet_loss, init_global_total_loss))              
        except:
            pass
    ...
    ckc_pointdir, log, batch_contrast_writer, batch_bce_writer, batch_total_writer, epoch_writer_tup, step_writer_tup = define_log_writer_for_triplewise(checkpoint, resume, args_txt, (model, model_name, image_size))
    ...
    global_triplet_loss = init_global_triplet_loss
    ...
    for epoch in range(init_epoch, epochs):
        ...
        model.train()
        print("Training...")
        for inputs0, fft_imgs0, labels0, inputs1, fft_imgs1, labels1, inputs2, fft_imgs2, labels2 in tqdm(dataloader_train):
            global_step += 1
            # Push to device
            inputs0, fft_imgs0, labels0 = inputs0.float().to(device), fft_imgs0.float().to(device), labels0.float().to(device)
            inputs1, fft_imgs1, labels1 = inputs1.float().to(device), fft_imgs1.float().to(device), labels1.float().to(device)
            ...
            # bceloss_1 = bce_loss(logps1, labels1)
            tripletloss = triplet_loss(embedding_0, embedding_1, embedding_2)
            loss = weight_importance * bceloss_0 + tripletloss

            if global_step % 500 == 0:
                print("Bceloss 0: {}  --- tripletloss: {} - Total: {} ".format(bceloss_0.item(), tripletloss.item(), loss.item()))
            ...
            global_triplet_loss += tripletloss.item()
            ...
            log.write_scalar(scalar_dict={"Tripletloss Loss/Single Step": tripletloss.item()}, global_step=global_step)
            ...
            batch_contrast_writer.write("{},{:.4f}\n".format(global_step, tripletloss.item()))
            ...
            if eval_per_iters != -1:
                if global_step % eval_per_iters == 0:
                    model.eval()
                    # Eval validation set
                    val_triplet_loss, val_bce_loss, val_total_loss, val_mac_acc, val_mic_acc, val_reals, val_fakes, val_micros, val_macros = eval_triplewise_dual_stream(model, weight_importance, dataloader_val, device, bce_loss, triplet_loss, adj_brightness=adj_brightness, adj_contrast=adj_brightness)
                    save_result_for_triplewise(step_val_writer, log, global_step, global_triplet_loss/global_step, global_bce_loss/global_step, global_total_loss/global_step, global_acc/global_step, val_triplet_loss, val_bce_loss, val_total_loss, val_mac_acc, val_mic_acc, val_reals, val_fakes, val_micros, val_macros, is_epoch=False, phase="val")
                    # Eval test set
                    test_triplet_loss, test_bce_loss, test_total_loss, test_mac_acc, test_mic_acc, test_reals, test_fakes, test_micros, test_macros = eval_triplewise_dual_stream(model, weight_importance, dataloader_test, device, bce_loss, triplet_loss, adj_brightness=adj_brightness, adj_contrast=adj_brightness)
                    save_result_for_triplewise(step_test_writer, log, global_step, global_triplet_loss/global_step, global_bce_loss/global_step, global_total_loss/global_step, global_acc/global_step, test_triplet_loss, test_bce_loss, test_total_loss, test_mac_acc, test_mic_acc, test_reals, test_fakes, test_micros, test_macros, is_epoch=False, phase="test")
                    # Save model:
                    ...
                    step_model_saver.save_model_for_pairwise(step_ckcpoint, model, global_step, save_ckcpoint=False, global_acc=global_acc, global_contrastive_loss=global_triplet_loss, global_bce_loss=global_bce_loss, global_total_loss=global_total_loss)
        ...
        # model.eval()
        # val_loss, val_mac_acc, val_mic_acc, val_reals, val_fakes, val_micros, val_macros = eval_triplewise_dual_stream(model, weight_importance, dataloader_val, device, bce_loss, triplet_loss, adj_brightness=adj_brightness, adj_contrast=adj_brightness)
        # save_result_for_triplewise(epoch_val_writer, log, epoch+1, running_loss/len(dataloader_train), running_acc/len(dataloader_train), val_loss, val_mac_acc, val_mic_acc, val_reals, val_fakes, val_micros, val_macros, is_epoch=True, phase="val")
        # # Eval test set
        # test_loss, test_mac_acc, test_mic_acc, test_reals, test_fakes, test_micros, test_macros = eval_triplewise_dual_stream(model, weight_importance, dataloader_test, device, bce_loss, triplet_loss, adj_brightness=adj_brightness, adj_contrast=adj_brightness)
        # save_result_for_triplewise(epoch_test_writer, log, epoch+1, running_loss/len(dataloader_train), running_acc/len(dataloader_train), test_loss, test_mac_acc, test_mic_acc, test_reals, test_fakes, test_micros, test_macros, is_epoch=True, phase="test")
        # # Save model:
