...
class FeedforwardNeuralNetModel(nn.Module):
    def __init__(self, input_dim=88, hidden_dim=256, output_dim=1024, dropout=0.0):
...
class PairwiseDualCNNFeedForwardViT(nn.Module):
    def __init__(self, \
                ...
                dropout_in_mlp=0.0):  
        super(PairwiseDualCNNFeedForwardViT, self).__init__()
    ...
    def forward_once(self, rgb_imgs, freq_features):
        ...
        z_fusion = self.fusion(x, y)
        z = self.mlp_head(z_fusion)
        out = self.sigmoid(z)
        return z_fusion, out

    def forward(self, rgb_imgs0, freq_features0, rgb_imgs1, freq_features1):
        embedding_0, out_0 = self.forward_once(rgb_imgs0, freq_features0)
        embedding_1, out_1 = self.forward_once(rgb_imgs1, freq_features1)
...
if __name__ == '__main__':
    ...
    model_ = PairwiseDualCNNFeedForwardViT(  image_size=128, num_classes=1, dim=1024,\
                                depth=6, heads=8, mlp_dim=2048,\
                                dim_head=64, dropout=0.15, emb_dropout=0.15,\
