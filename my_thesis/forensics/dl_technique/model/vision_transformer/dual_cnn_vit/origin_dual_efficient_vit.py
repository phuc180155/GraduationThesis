...
class OriginDualEfficientViT(nn.Module):
    def __init__(self, channels=1280,\
                 ...
                 pretrained=False, dropout_in_mlp=0.0): 

        super(OriginDualEfficientViT, self).__init__()
        ...
        self.spatial_extractor = self.get_feature_extractor(freeze=freeze, pretrained=pretrained, num_classes=num_classes, in_channels=3)   # efficient_net-b0, return shape (1280, 8, 8) or (1280, 7, 7)
        self.freq_extractor = self.get_feature_extractor(freeze=freeze, pretrained=pretrained, num_classes=num_classes, in_channels=1)
    ...
    def get_feature_extractor(self, architecture="efficient_net", freeze=0, pretrained=True, num_classes=1, in_channels=3):
        extractor = None
        if architecture == "efficient_net":
            if isinstance(pretrained, int):
                extractor = EfficientNet.from_pretrained('efficientnet-b0', num_classes=num_classes,in_channels = in_channels, pretrained=pretrained)
            else:
...
if __name__ == '__main__':
    ...
    model_ = OriginDualEfficientViT(image_size=256, patch_size=2)
    out = model_(x, y)
    print(out.shape)