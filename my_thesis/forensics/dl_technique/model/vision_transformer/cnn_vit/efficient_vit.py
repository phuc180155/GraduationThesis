...
class EfficientViT(nn.Module):
    def __init__(self, channels=1280, selected_efficient_net=0,\
                 ...
                 pretrained=True, freeze=0, pool='cls', dropout_in_mlp=0.0):
        super().__init__()
        ...
        if selected_efficient_net == 0:
            ...
            self.extractor.load_state_dict({re.sub("^module.", "", k): v for k, v in state_dict.items()},
                                               strict=False)
        if freeze:
            for i in range(0, len(self.extractor._blocks)):
                for index, param in enumerate(self.extractor._blocks[i].parameters()):
                    if i >= len(self.extractor._blocks) - 3:
    ...
    def forward(self, img, mask=None):
        p = self.patch_size
        x = self.extractor.extract_features(img)  # 1280x7x7
